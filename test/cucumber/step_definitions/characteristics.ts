...
Given('vehiculos con las siguientes caracteristicas:', async function (this: MyWorld, dataTable: DataTable) {
  const rows = dataTable.hashes()
  const created: Array<{ id: string; plate: string; characteristics: Characteristic[] }> = []

  for (const row of rows) {
    const plate = row['patente']
    const characteristics = parseCharacteristics(row['caracteristicas'] ?? '')
    const createRes = await api(this).post('/vehicle', buildVehicleDto(plate))
...
Given('un vehiculo con las caracteristicas {string}', async function (this: MyWorld, raw: string) {
  const characteristics = parseCharacteristics(raw)
  const createRes = await api(this).post('/vehicle', buildVehicleDto(`AA${Date.now()}AA`))
  expect(createRes.status).toBe(201)
...
When('actualizo las caracteristicas del vehiculo a {string}', async function (this: MyWorld, raw: string) {
  const characteristics = parseCharacteristics(raw)
  this.world.current_characteristics = characteristics
  const vehicleId = this.world.create_vehicle_response.body.id
  this.world.update_vehicle_response = await api(this).patch(`/vehicle/${vehicleId}`, {
    characteristics,
  })
})

When('elimino la caracteristica {string}', async function (this: MyWorld, raw: string) {
...
Then('solo aparecen vehiculos con la caracteristica {string}', function (this: MyWorld, raw: string) {
  ...
  for (const vehicle of response.body) {
    expect(vehicle.characteristics).toEqual(expect.arrayContaining([expectedCharacteristic]))
  }
...
Then('el vehiculo no tiene la caracteristica {string}', async function (this: MyWorld, raw: string) {
