...
const rawToUpdateDto = (rawData: Record<string, string>) => {
    ...
    if ('fotos' in rawData) dto.photos = rawData['fotos'].split(',').map((s: string) => s.trim());
...
Given('el vehiculo ya esta publicado', async function (this: MyWorld) {
    ...
    this.world.create_vehicle_response = await api(this)
        .post("/vehicle", this.world.create_vehicle_dto);
...
Given('el vehículo esta deshabilitado', async function (this: MyWorld) {
    const response = await api(this)
        .patch(`/vehicle/${this.world.create_vehicle_response.body.id}`, { enabled: false });
...
When('actualizo la información de un vehículo con los siguientes datos:', async function(this: MyWorld, dataTable: DataTable) {
    ...
    this.world.update_vehicle_response = await api(this) 
        .patch(`/vehicle/${this.world.create_vehicle_response.body.id}`, this.world.update_vehicle_dto);
});
...
When('elimino el vehículo', async function(this: MyWorld) {
    ...
    this.world.delete_vehicle_response = await api(this)
        .delete(`/vehicle/${vehicleId}`);
});

When('habilito el vehículo', async function(this: MyWorld) {
    const vehicleId = this.world.create_vehicle_response.body.id;
    this.world.enable_vehicle_response = await api(this)
        .patch(`/vehicle/${vehicleId}`, { enabled: true });
});

When('deshabilito el vehículo', async function(this: MyWorld) {
    const vehicleId = this.world.create_vehicle_response.body.id;
    this.world.update_vehicle_response = await api(this)
        .patch(`/vehicle/${vehicleId}`, { enabled: false });
...
Then("el vehículo no aparece en 'Mis vehículos'", async function(this: MyWorld) {
    const my_vehicles_response = await api(this)
        .get(`/vehicle`);
...
Then("el vehiculo queda actualizado", async function(this: MyWorld) {
    ...
    const vehicleId = this.world.create_vehicle_response.body.id;
    const get_vehicle_response = await api(this)
...
Then('el sistema indica que no se puede modificar el campo {string}', async function(this: MyWorld, campo: string) {
    expect(this.world.update_vehicle_response.status).toBe(400);
    const field = COLUMN_MAP[campo.toLowerCase()]
    expect(this.world.update_vehicle_response.body.message?.toLowerCase()).toContain(field);
});
